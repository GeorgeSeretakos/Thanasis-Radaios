...
export default function ServiceDetailPage({ params }) {
  ...
  const faqHref =
    typeof svc.faq === "string" && svc.faq.trim() ? `/faq#${svc.faq}` : null;

  return (
    <main className="px-4">
      {/* Sticky mini-nav under the fixed navbar (navbar height ≈ 64px = top-16) */}
      <div className="sticky top-16 z-40 border-b border-white/10 bg-[#0B0B0C]/85 backdrop-blur">
        <nav className="max-w-5xl mx-auto py-3 text-sm">
      ...
      <section className="max-w-5xl mx-auto pb-20">
        {svc.aHtml ? (
          <div
            ...
            dangerouslySetInnerHTML={{ __html: svc.aHtml }}
          />
        ) : (
