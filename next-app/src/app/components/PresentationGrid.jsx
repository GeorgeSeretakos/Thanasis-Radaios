// components/PresentationGrid.jsx
export default function PresentationGrid({ items = [] }) {
  const imgHeightClasses = "h-36 md:h-48 lg:h-56";

  return (
    <section className="w-full py-12">
      <div className="w-9/10 mx-auto grid grid-cols-1 md:grid-cols-4 gap-2">
        {items.map((it, idx) => {
          // highlight the 2nd card (index 1)
              ...
              {it.image && (
                <div
                  className={[
                    ...
                    imgHeightClasses,
                    "bg-gray-50",
                    isEmphasized ? "border-b border-[#1C86D1]" : "border-b border-[#1C86D1]/30",
                  ].join(" ")}
                >
                  <img
                    src={it.image}
